...
if (mini) {
  ...
  sample_r_theta = sample(sample_r_theta, round(length(sample_r_theta)/5), replace=TRUE)
  sample_identify_pref = sample(sample_identify_pref, round(length(sample_identify_pref)/5), replace=TRUE)
  sample_identify_theta = sample(sample_identify_theta, length(sample_identify_theta), replace=TRUE)
...
# initial_param_trial[x_transform[[2]]$beta_theta_ind[1]] = log(0.01);
    ...
    output_wrt_r = function(x_transform, silent = TRUE) {
      ...
      compute_deriv = function(mini_data_index) {
        ...
        f_output = function(output_hh) {
          return(list(sum((output_hh$m - m_observed)^2) + sum((c(output_hh$m %*% t(output_hh$m)) - c(m_observed %*% t(m_observed)))^2), sum(output_hh$vol_sts_counterfactual)))
        }
      ...
      output = output_1 + (abs(output_2_sqrt) > 0.04)*1000 + (abs(output_3_sqrt) > 3e-3)*1000;
      deriv = deriv_1;
      print('------VOLUNTARY HH---------')
