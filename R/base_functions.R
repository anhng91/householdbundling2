...
#' @param full_output logical value, if FALSE, return values conditional on draws of theta
#'
...
#' 
#' 
moment_ineligible_hh = function(data_set, param, full_output=FALSE) {
	tol = 1e-3;
	...
	for (mem_index in 1:HHsize) {
		...
		if (!(full_output)) {
			...
			m_draw[,mem_index] = theta_draw[,mem_index] * kappa_draw[,mem_index] + obj2 * theta_draw[,mem_index] * obj3 * obj1 * kappa_draw[,mem_index] * (data_set$R_draw[[1]] > 0)
		}	
	}
...
counterfactual_household_draw_theta_kappa_Rdraw = function(hh_index, param, n_draw_halton = 1000, n_draw_gauss = 10, sick_parameters, xi_parameters, u_lowerbar = -10, policy_mat_hh, seed_number=1, constraint_function, within_hh_heterogeneity = list(omega=TRUE, gamma=TRUE, delta=TRUE, theta_bar=TRUE), income_vec = NA, contraction_variance = 1, compute_WTP=TRUE, derivative=FALSE, numerical_derivative=NA, option_derivative = 'beta_theta_ind', always_covered=FALSE, penalty = 0, risk_discrimination_dummy = FALSE, risk_discrimination = function(x) 0, afford=TRUE, compute_wtp2 = TRUE, compute_counterfactual_cost = FALSE, multiple_prices = FALSE) {
	...
	if (data_hh_i$HHsize_s[1] > 0) {
		...
		if (compute_WTP) {
			if (max(theta_draw) == 0) {
				wtp = 0; 
				cost_to_insurance = rep(0, HHsize)
				...
				if (compute_wtp2) {
					for (i in elig_member_index) {
						...
						if (compute_counterfactual_cost) {
							cost_to_insurance[i]  = colMeans(m_fun(list(theta_draw = theta_draw, kappa_draw = kappa_draw_uni[[i]], R_draw = un_censored_R_uni[[i]] + (income_vec[1] - income_vec[2]), omega = omega, gamma = gamma, delta = delta, HHsize = HHsize), income_effect = income_effect)$insurer_cost)[i];
						}
					}
