...
#' @param n_simulation_r an integer for the number of Gauss Laguerre draws. 
#'
...
#' 
#' @importFrom pracma gaussLaguerre
#' @export
#'
#' @examples
#' household_draw_theta_kappa_Rdraw(1, sample_data_and_parameter$param, 1000, 10, sick_parameters_sample, xi_parameters_sample)
household_draw_theta_kappa_Rdraw = function(hh_index, param, n_draw_halton = 1000, n_draw_gauss = 10, sick_parameters, xi_parameters, u_lowerbar = -10, short=TRUE, derivative_r_threshold = FALSE, n_simulation_r = 100) {
	set.seed(1);
	tol = 1e-4;
	...
	r_draw = gaussLaguerre(n_simulation_r, 0);
	...
	if (data_hh_i$HHsize_s[1] > 0) {
		...
		for (j in 1:n_draw_halton) {
			...
			cara = function(x,r) {
				if (length(r) > 1) {
					return(lapply(r, function(ri) ifelse(ri == 0, mean(x, na.rm=TRUE), mean((1-exp(-r * x))/r, na.rm=TRUE))) %>% unlist())
				} else {
					if (r != 0) {
			...
			if (data_hh_i$N_vol[1] == data_hh_i$HHsize_s[1]) {
				for (i in elig_member_index) {
					...
					if (loop_index == 0) {
						realized_insurance_r = lapply(r_draw, function(r) cara(U_full_insurance,r) >= cara(U_deviate[[i]], r)) %>% unlist()
					} else {
						realized_insurance_r = realized_insurance_r * (lapply(r_draw, function(r) cara(U_full_insurance,r) >= cara(U_deviate[[i]], r)) %>% unlist())
					}
				}
			} else {
				for (i in elig_member_index) {
					...
					if (loop_index == 0) {
						...
						realized_insurance_r = realized_insurance_r * (lapply(r_draw, function(r) cara(U_uninsured,r) >= cara(U_deviate[[i]], r)) %>% unlist())
					}
				}
			}
			realized_outcome[[j]] = realized_insurance_r			
		}
	}
	...
	if (data_hh_i$HHsize_s[1] > 0) {
		...
		output$X_hh = var_hh(data_hh_i);
		output$realized_insurance_r = realized_outcome;
	} else {
		output = list(); 
